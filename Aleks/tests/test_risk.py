...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Bash", {"command": r"find . -exec rm -rf {} \;"}, True),
        ("Bash", {"command": "find . -name '*.tmp' | xargs rm -rf"}, True),
        ("Bash", {"command": 'bash -c "rm -rf /x"'}, True),
        ("Bash", {"command": 'sh -c "rm -rf /x"'}, True),
        ("Bash", {"command": "eval rm -rf /x"}, True),
        ("Bash", {"command": "FOO=bar rm -rf /x"}, True),
        ("Bash", {"command": "nohup rm -rf /x &"}, True),
        ("Bash", {"command": "sudo systemctl restart nginx"}, True),
        ("Bash", {"command": "docker compose up -d"}, True),
        ("Bash", {"command": "bash deploy.sh"}, True),
        ("Bash", {"command": "python deploy.py"}, True),
        ("Bash", {"command": "cat deploy_notes.txt"}, False),
        ("Bash", {"command": "ls deploy/"}, False),
